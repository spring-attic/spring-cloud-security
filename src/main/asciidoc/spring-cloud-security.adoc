...
> Warning: If you use the `oauth2.resource.jwt.keyUri` the
> authorization server needs to be running when your application
> starts up. It will log a warning if it can't find the key, and tell
...
authentication). The `oauth2.client.*` properties are bound to an instance 
